...
export class AnimeAnimator {
  constructor(parentElement, animationDescription, options) {
    this.options = {
      controls: true,
      autoplay: false,
      useKeyframes: false,
  ...
  initTimeline() {
    this.timeline = anime.timeline({
      ...
      update: () => {
        ...
        this.timeDisplay.innerText = `${Math.round(this.timeline.progress / 1e4 * this.timeline.duration) / 10} / ${Math.round(this.timeline.duration / 100) / 10}`;
        if (this.playFragment) {
          const now = this.timeline.progress * 25;
  ...
  resize() {
    const progress = this.timeline.progress * this.timeline.duration * 0.01;
  ...
  createControls() {
    ...
    const nextButton = createHTMLElement(`<div id='nextButton' class='button icon next'></div>`);
    const playButton = createHTMLElement(`<div id='playButton' class='button icon play'></div>`);
    ...
    const timestamps = createHTMLElement(`<select id='timestamps' class='button'><option value='0'>timestamps</option></select>`);
    this.timestampSelect = timestamps;
    this.timestamps.forEach((ts) => {
      const $ = createHTMLElement(`<option value='${ts.time}' class='button'>${ts.name}</option>`);
    ...
    timeDisplay.innerHTML = `${Math.round(this.timeline.progress / 1e4 * this.timeline.duration) / 10} / ${Math.round(this.timeline.duration / 100) / 10}`;
